...
# Configuration
_TRANSCRIPT_SESSION_STAMP = datetime.now().strftime("%Y%m%d_%H%M%S")

CONFIG = {
    "expected_validated_tasks": 1,  # Number of validated tasks we want
    "max_tasks_to_generate": 2,    # Maximum tasks to generate before stopping
    "model_for_generation": "Qwen/Qwen2.5-3B-Instruct",
    ...
    # NEW: transcript saving
    "save_transcripts": True,
    "transcripts_dir": "transcripts",
    "save_transcript_conditions": ["complete", "hidden"],  # any of: "complete", "hidden"
    # NEW: also save transcripts for tasks that fail validation (costs extra runtime)
    "save_transcripts_for_failed_tasks": True,
}

def _resolve_transcripts_dir() -> str:
    """Resolve transcripts_dir to an absolute path anchored at this file."""
    base = CONFIG["transcripts_dir"]
    if os.path.isabs(base):
        resolved_base = base
    else:
        here = os.path.dirname(os.path.abspath(__file__))
        resolved_base = os.path.join(here, base)

    # Append DAY-TIME folder
...
def run_full_scenario_with_logging(task, is_complete, prompts):
    """
    Run full scenario (like run_full_scenario) but also record discussion per round,
    and return a run_data dict suitable for saving as a transcript.
    """
    model = CONFIG["model_for_simulation"]
    num_rounds = CONFIG["num_rounds"]
    extra_prompt = CONFIG["extra"]
    ...
    agents = []
    hidden_info = list(hidden_info)  # avoid mutating caller
    random.shuffle(hidden_info)

    for i in range(num_agents):
        information_list = shared_info.copy()
        if not is_complete:
            information_list.append(hidden_info[i])
        else:
            information_list.extend(hidden_info)
    ...
    # Discussion rounds (record messages)
    for round_num in range(num_rounds):
        round_msgs = []
        ...
        if round_num == 0:
            ...
            for agent_idx, agent in enumerate(agents):
                initial_vote_data = run_data["initial_votes"][agent_idx]
                initial_vote_prefix = (
                    f"Your initial vote was: {initial_vote_data['vote']}\n"
                    f"Your initial rationale was: {initial_vote_data['rationale']}\n\n"
                )
                ...
                prev_messages.append(f"{agent.name}: {response_str}")
                round_msgs.append({"agent": agent.name, "message": response_str})
        else:
            for agent in agents:
                prev_messages = []
                current_idx = agents.index(agent)
                ...
                response = agent.chat(prompt)
                round_msgs.append({"agent": agent.name, "message": str(response)})

        run_data["discussion_rounds"].append(round_msgs)

    # Final votes
    final_messages = [f"{agent.name}: {agent.history[-1]['content']}" for agent in agents]
    group_discussion = "\n".join(final_messages)
    ...
    # (Optional) quick sanity stats (not returned, but kept here if you want later)
    _initial_accuracy = (
        sum(1 for v in run_data["initial_votes"] if v["vote"] == correct_answer) / len(run_data["initial_votes"])
        if run_data["initial_votes"] else 0.0
    )
    _final_accuracy = (
        sum(1 for v in run_data["final_votes"] if v["vote"] == correct_answer) / len(run_data["final_votes"])
        if run_data["final_votes"] else 0.0
    )
...
def _format_transcript(task: dict, runs: list[dict]) -> str:
    """Create a human-readable transcript string for one task."""
    lines: list[str] = []
    ...
    for a in task.get("possible_answers", []):
        lines.append(f"  - {a}")
    lines.append("")
    lines.append(f"Correct Answer: {task.get('correct_answer', '')}")
    lines.append("")
    lines.append("Shared Information:")
    for i, fact in enumerate(task.get("shared_information", []), start=1):
        lines.append(f"  Fact {i}: {fact}")
    lines.append("")
    lines.append("Hidden Information (pool):")
    for i, fact in enumerate(task.get("hidden_information", []), start=1):
        lines.append(f"  Hidden {i}: {fact}")
    lines.append("")
    lines.append("=" * 100)
    lines.append("")

    for run in runs:
        cond = "COMPLETE" if run.get("is_complete") else "HIDDEN"
        lines.append(f"CONDITION: {cond}")
        lines.append("-" * 100)

        # Initial votes
        lines.append("Initial votes:")
        for v in run.get("initial_votes", []):
            lines.append(f"  {v['agent']} voted: {v['vote']}")
            lines.append(f"    Rationale: {v['rationale']}")
        lines.append("")

        # Discussion rounds
        rounds = run.get("discussion_rounds", [])
        if rounds:
            lines.append("Discussion:")
            for r_idx, r_msgs in enumerate(rounds, start=1):
                lines.append(f"  Round {r_idx}:")
                for msg in r_msgs:
                    lines.append(f"    {msg['agent']}: {msg['message']}")
                lines.append("")
        else:
            lines.append("Discussion: (none captured)")
            lines.append("")
...
def _save_task_transcript(task: dict, runs: list[dict]) -> str:
    """Write transcript to .../taskX.txt and return filepath."""
    transcripts_dir = _resolve_transcripts_dir()
    os.makedirs(transcripts_dir, exist_ok=True)

    task_id = task.get("id")
    if task_id is None:
        raise ValueError("Task is missing 'id' needed for filename taskX.txt")

    out_path = os.path.join(transcripts_dir, f"task{task_id}.txt")
...
def main():
    ...
    while (len(validated_tasks) < CONFIG['expected_validated_tasks'] and
        ...
        if task:
            ...
            # Decide whether to save transcript
            should_save_transcript = bool(
                CONFIG.get("save_transcripts", False)
                and (
                    validation_result.get("is_valid", False)
                    or CONFIG.get("save_transcripts_for_failed_tasks", False)
                )
            )
            ...
            # Save transcript (validated-only by default; toggle via save_transcripts_for_failed_tasks)
            if should_save_transcript:
                runs_for_file = []
                conds = set(CONFIG.get("save_transcript_conditions", ["complete", "hidden"]))
                if "complete" in conds:
                    runs_for_file.append(run_full_scenario_with_logging(task, True, prompts))
                if "hidden" in conds:
                    runs_for_file.append(run_full_scenario_with_logging(task, False, prompts))

                transcript_path = _save_task_transcript(task, runs_for_file)
